...
if "marca_actual" not in st.session_state:
    st.session_state.marca_actual = None

# --- PANTALLA DE INGRESO (LOGIN) ---
if st.session_state.usuario_actual is None:
    st.title("Bienvenido a NotPed")
    st.write("Plataforma exclusiva para gestión de pedidos mayoristas.")
        ...
        with st.form("login_form"):
            ...
            if submit_login:
                if usr and pwd:
                    ...
                    if len(res.data) > 0:
                        datos_usr = res.data[0]
                        st.session_state.usuario_actual = datos_usr["usuario"]
                        st.session_state.rol_actual = datos_usr["rol"]
                        st.session_state.marca_actual = datos_usr["nombre_marca"]
                        st.rerun()
                    else:
...
# --- SISTEMA PRINCIPAL (SI YA INGRESÓ) ---
else:
        ...
        if productos:
            ...
            for index, prod in enumerate(productos):
                ...
                with columna_actual:
                    ...
                    # Si es proveedor, o si queremos que el revendedor vea el precio
